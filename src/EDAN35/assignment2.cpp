...
edan35::Assignment2::run()
{
    ...
    auto const quad_geometry = loadQuad();
    Node quad;
    quad.set_geometry(quad_geometry);
    /**
	...
	GLuint rain_shader = 0u;
	program_manager.CreateAndRegisterProgram("Rain",
	                                         { { ShaderType::vertex, "EDAN35/fill_gbuffer.vert" },
	                                           { ShaderType::fragment, "EDAN35/rain.frag" } },
	                                         rain_shader);
	if (rain_shader == 0u) {
		LogError("Failed to load rain shader");
		return;
	}
	GBufferShaderLocations rain_shader_locations;
	fillGBufferShaderLocations(rain_shader, rain_shader_locations);
	...
	for (size_t i = 0; i < static_cast<size_t>(lights_nb); ++i) {
		...
		lightColors[i] = glm::vec3(0.0f + 0.5f * (static_cast<float>(rand()) / static_cast<float>(RAND_MAX)),
		                           0.0f + 0.5f * (static_cast<float>(rand()) / static_cast<float>(RAND_MAX)),
		                           0.0f + 0.5f * (static_cast<float>(rand()) / static_cast<float>(RAND_MAX)));
	}
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (!shader_reload_failed) {
			...
			glClear(GL_DEPTH_BUFFER_BIT | GL_COLOR_BUFFER_BIT); // changed from glClear(GL_DEPTH_BUFFER_BIT)
			...
			for (std::size_t i = 0; i < sponza_geometry.size(); ++i)
			{
				...
				glUniform1i(glGetUniformLocation(fill_gbuffer_shader, "is_floor"),
						geometry.name == "sponza_18" || geometry.name == "sponza_117");
			...
			if (true) {
				glEnable(GL_BLEND);
				//glDepthMask(GL_FALSE); // look at depth buffer but don't change it
				glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);  
				glUseProgram(water_shader);
				glUniform1i(water_shader_locations.diffuse_texture, 0);
				glUniform1i(water_shader_locations.specular_texture, 1);
				...
				auto const& geometry = water_geometry;

				utils::opengl::debug::beginDebugGroup(geometry.name);
				
				TRSTransformf tf;
				tf.SetTranslate({-1.5e3, water_level, -0.8e3});
				tf.SetScale({4, 1, 2});
				auto const vertex_model_to_world = tf.GetMatrix();
				auto const normal_model_to_world = glm::mat4(1.0f);
				
				glUniform1f(glGetUniformLocation(water_shader, "ellapsed_time_s"), elapsed_time_s);

				glUniformMatrix4fv(water_shader_locations.vertex_model_to_world, 1, GL_FALSE, glm::value_ptr(vertex_model_to_world));
				glUniformMatrix4fv(water_shader_locations.normal_model_to_world, 1, GL_FALSE, glm::value_ptr(normal_model_to_world));
				/*
				glUniform1i(water_shader_locations.has_normals_texture, 1);
				glBindSampler(2u, mipmap_sampler);
				glActiveTexture(GL_TEXTURE2);
				glBindTexture(GL_TEXTURE_2D, waves_normal);
				*/
	        ...
	        // DRAW PARTICLES
	        //
			glBeginQuery(GL_TIME_ELAPSED, elapsed_time_queries[toU(ElapsedTimeQuery::Particles)]);
			if (true) {
				glEnable(GL_BLEND);
				glDepthMask(GL_FALSE); // look at depth buffer but don't change it
				glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);  
				glUseProgram(rain_shader);
				glUniform1i(rain_shader_locations.diffuse_texture, 0);
				glUniform1i(rain_shader_locations.specular_texture, 1);
				glUniform1i(rain_shader_locations.normals_texture, 2);
				glUniform1i(rain_shader_locations.opacity_texture, 3);
				utils::opengl::debug::beginDebugGroup("Draw particles");
				rain.step();

				for (raindrop &drop : rain.drops) {
	                /** TODO Matrices wrong, rain should start from the top and fall down,
	                set_uniforms doesn't do anything yet. Check how to render many particles*/
					/**particle.render(view_projection,
					            drop.get_transform(),
					            phong_shader, phong_set_uniforms); */
					
					auto const& geometry = particle_geometry;

					utils::opengl::debug::beginDebugGroup(geometry.name);
					
					TRSTransformf tf = drop.get_transform();
					auto const vertex_model_to_world = tf.GetMatrix();
					auto const normal_model_to_world = glm::transpose(tf.GetMatrixInverse());

					glUniformMatrix4fv(rain_shader_locations.vertex_model_to_world, 1, GL_FALSE, glm::value_ptr(vertex_model_to_world));
					glUniformMatrix4fv(rain_shader_locations.normal_model_to_world, 1, GL_FALSE, glm::value_ptr(normal_model_to_world));
			...
			for (size_t i = 0; i < static_cast<size_t>(lights_nb); ++i) {
				...
				// XXX: Is any clearing needed? YES?
...
loadQuad()
{
	bonobo::mesh_data quad;
	quad.vertices_nb = 4;
	quad.drawing_mode = GL_TRIANGLE_STRIP;
	float vertexArrayData[4 * 3] = {
		1.f, 0.f, 0.f,
		1.f, 1.f, 0.f,
		0.f, 0.f, 0.f,
		0.f, 1.f, 0.f,
	...
	{
		utils::opengl::debug::nameObject(GL_VERTEX_ARRAY, quad.vao, "Quad VAO");

		glGenBuffers(1, &quad.bo);
		assert(quad.bo != 0u);
		glBindBuffer(GL_ARRAY_BUFFER, quad.bo);
		glBufferData(GL_ARRAY_BUFFER, quad.vertices_nb * 3 * sizeof(float), vertexArrayData, GL_STATIC_DRAW);
		utils::opengl::debug::nameObject(GL_BUFFER, quad.bo, "Quad VBO");
